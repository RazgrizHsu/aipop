...
class winPop: NSWindow, NSWindowDelegate
{
	...
	func resetPositions()
	{
		...
		log( "[wpop] monX[ \(monX) ] inDic[\( String(describing: inDic) )]" )
		...
		{
			lastMonX = monX
			log( "[wpop] isFirst[ \(isFirst) ] isMonCh[ \(isMonCh) ]" )
			
			if let ofm = dic[monX] {
				if( ofm.size.width != 0 && ofm.size.height != 0 ) { self.setContentSize( ofm.size ) }
